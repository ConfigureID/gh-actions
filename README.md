...
jobs:
  build:
    uses: ConfigureID/gh-actions/.github/workflows/npm-build.yml@v18
    with:
      node_version: 14
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Build project
        uses: ConfigureID/gh-actions/npm-build@v18
        with:
          node_version: 14
          # Directory where this project is built (some use dist, others build, etc)
          dist_dir: dist
...
jobs:
  deploy:
    ...
    steps:
      - name: Deploy branch
        uses:  ConfigureID/gh-actions/deploy@v18
        with:
  ...
  prune:
    ...
    steps:
      ...
      - name: Prunes enviroment and files
        uses: ConfigureID/gh-actions/gh-prune@v18
        with:
...
jobs:
  release:
    ...
    steps:
      ...
      - name: Create release
        uses: ConfigureID/gh-actions/release@v18
        with:
...
jobs:
  e2e-remote:
    ...
    steps:
      - name: E2E Test
        uses: ConfigureID/gh-actions/cypress@v18
        with:
...
jobs:
  e2e-remote:
    ...
    steps:
      - name: E2E Test
        uses: ConfigureID/gh-actions/playwright@v18
        with:
          # Test on the branch deployment on GCP
          base_url: ${{ format('https://{0}/{1}/branches/{2}', vars.BASE_URL, vars.NAMESPACE, github.event.pull_request.head.ref) }}
          NPM_TOKEN: ${{ secrets.NPM_TOKEN }}
...
jobs:
   test-reports:
    ...
    steps:
      - name: Create reports
        uses: ConfigureID/gh-actions/test-report-pr@v18
   ...
   test-reports:
    ...
    steps:
      - name: Upload E2E Test HTML reports
        uses: ConfigureID/gh-actions/test-report-cloud@v18
   ...
   read-version:
    ...
    steps:
      - name: Get JSON "version" property value
        uses: ConfigureID/gh-actions/get-remote-json-property@v18
        id: get-version
        with: 
   ...
   read-version:
    ...
    steps:
      - name: Get deployed version
        uses: ConfigureID/gh-actions/get-version@v18
        id: deployed-version
        with: 
   ...
   read-version:
    ...
    steps:
      - name: Get current version in staging
        uses: ConfigureID/gh-actions/compare-version@v18
        id: compare-version
        with: 
          version: v1.4.3
          base_url: somedomain.com/apps
...
jobs:
   read-version:
    ...
    steps:
      ...
      - name: Upload artifact as release asset
        uses: ConfigureID/gh-actions/attach-release-artifact@v18
        with:
          release_name: v1.4.3
